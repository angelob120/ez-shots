...
// expired, cancelled) and the six numbers for the top. This file sorts them
// into Needs attention, Today, Upcoming and Past, draws a card each, and
// PATCHes the three things the owner can do to one: mark it paid, cancel it,
// or leave a private note. Signing in lives in js/admin-core.js.
(function () {
  ...
  function card(b, withDay) {
    ...
    return '<article class="bk bk-' + esc(b.state) + '" data-id="' + esc(b.id) + '">' +
      '<div class="bk-head"><b>' + esc(head) + '</b><span class="pill pill-' + esc(b.state) + '">' + esc(LABEL[b.state] || b.state) + "</span></div>" +
  ...
  document.addEventListener("click", function (e) {
    ...
    if (act === "cancel" && !window.confirm("Cancel " + id + "? The slot opens up again straight away. Any refund is yours to do in Stripe.")) return;
    btn.disabled = true;
    status(s, "pending", "Saving...");
    api("/api/admin/bookings/" + encodeURIComponent(id), { method: "PATCH", body: JSON.stringify(body) })
      .then(function () { status(s, "success", act === "note" ? "Note saved." : "Done."); return load(); })
